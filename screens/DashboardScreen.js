...
export default function DashboardScreen({ navigation }) {
  ...
  const loadDashboard = async () => {
    ...
    const since = getDaysSince(last.bookingDate);
    setDaysSince(since);
    ...
    if (daysLeft <= 7 && daysLeft > 0) {
      setSuggestion('Book soon! Only ~' + daysLeft + ' days of gas left.');
      // Alert = Broadcasting equivalent
      Alert.alert('Low Gas Alert!', 'Only ~' + daysLeft + ' days remaining. Book now!');
    } else if (daysLeft === 0) {
      setSuggestion('You may have run out! Book a cylinder now.');
    } else {
      setSuggestion("You're good! ~" + daysLeft + ' days of gas left.');
    }
  ...
  const bookCylinder = async () => {
    ...
    setEmptyDate('');
    setSlot('');
    Alert.alert('Booked!', 'Cylinder booked for ' + today + '!');
    loadDashboard();
  };
      ...
      <ScrollView contentContainerStyle={styles.body}>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statBox}>
            <Text style={styles.statLabel}>Days since booking</Text>
